...
function matchRecurrence(everyDays: number): string {
  const m = RECURRENCE_OPTIONS.find((o) => o.days === everyDays);
  return m?.value ?? "90";
}

export function EditTaskSheet() {
  const taskId = useUIStore((s) => s.editTaskId);
  const close = useUIStore((s) => s.closeEditTask);
  ...
  useEffect(() => {
    ...
    setRecurrence(
      task.recurrence.kind === "once" ? "once" : matchRecurrence(task.recurrence.everyDays),
    );
    setDueDate(new Date(task.nextDueAt).toISOString().slice(0, 10));
    setNotes(task.notes ?? "");
  }, [task]);

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!task || !title.trim() || !propertyId) return;
  ...
  return (
    <Sheet open={!!taskId} onOpenChange={(o) => !o && close()}>
      <SheetContent side="right" className="w-full sm:max-w-md bg-paper p-0 overflow-y-auto [&>button]:hidden">
        {task && (
          <form onSubmit={submit} className="p-8 flex flex-col h-full">
            ...
            <div className="mb-7">
              ...
              <h2 className="font-display font-semibold text-2xl mt-1 text-balance">
                {task.title}
              </h2>
            </div>
